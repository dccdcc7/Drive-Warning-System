...
class PerspectiveTransformation(object):
    ...
    def __init__(self, img_size=(1280, 720), logger=None ):
        ...
        # use self.img_size consistently and keep integer coordinates
        offset_x = int(self.img_size[0] / 4)
        offset_y = 0
        w, h = int(self.img_size[0]), int(self.img_size[1])
        self.dst = np.float32([(offset_x, offset_y),
                    (offset_x, h - offset_y - 1),
                    (w - offset_x - 1, h - offset_y - 1),
                    (w - offset_x - 1, offset_y)])
    ...
    def updateTransformParams(self, left_lanes: Union[list, np.ndarray], right_lanes: Union[list, np.ndarray], type: str = "Default") -> None :
        ...
        if (len(left_lanes) and len(right_lanes)) :
            ...
            # ensure integer coordinates and clamp to image bounds
            def clamp(pt):
                x = int(max(0, min(int(pt[0]), int(self.img_size[0]) - 1)))
                y = int(max(0, min(int(pt[1]), int(self.img_size[1]) - 1)))
                return (x, y)

            self.src = np.float32([clamp(top_left), clamp(bottom_left), clamp(bottom_right), clamp(top_right)])
            self.M = cv2.getPerspectiveTransform(self.src, self.dst)
            self.M_inv = cv2.getPerspectiveTransform(self.dst, self.src)
    ...
    def transformToBirdView(self, img: cv2, flags=cv2.INTER_LINEAR):
        ...
        # OpenCV expects dsize as (width, height) tuple of ints
        dsize = (int(self.img_size[0]), int(self.img_size[1]))
        return cv2.warpPerspective(img, self.M, dsize, flags=flags)
    ...
    def calcCurveAndOffset(self, img: cv2, left_lanes: np.ndarray, right_lanes: np.ndarray) -> Tuple[Tuple, float]:
        ...
        if (len(left_lanes) and len(right_lanes)) :
            ...
            if  left_lanes[0, 0] <= left_lanes[ int(len(left_lanes)/2), 0]:
                curvature_direction = "L"
            elif  right_lanes[0, 0] >= right_lanes[ int(len(right_lanes)/2), 0]:
                curvature_direction = "R"
            else :
                curvature_direction = "F"
