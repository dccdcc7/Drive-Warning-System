...
class SingleCamDistanceMeasure(object):
    ...
    INCH = 0.39*scale
    RefSizeDict = {
        "person": (160 * INCH, 50 * INCH),  # 定义不同车辆的尺寸，用于像素尺寸和真实距离的映射
    ...
    def DrawDetectedOnFrame(self, frame_show: cv2) -> None:
        if (len(self.distance_points) != 0):
            for x, y, d in self.distance_points:
                cv2.circle(frame_show, (x, y), 4, (255, 255, 255), thickness=-1) # 在每个目标的中心画个圆点
                ...
                cv2.putText(frame_show, text, (textX + 1, textY + 5), fontFace=cv2.FONT_HERSHEY_TRIPLEX,
                            fontScale=fontScale,
                            color=(0, 255, 0), thickness=2)
                # putText_shadow(frame_show, text, (textX + 1, textY + 5), fontFace=cv2.FONT_HERSHEY_TRIPLEX,
                #                fontScale=fontScale,
                #                color=(0, 255, 0), thickness=1, shadow_color=(255, 255, 255))
